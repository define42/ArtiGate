...
The `Dockerfile` is a two-stage build, both stages on `golang:1.27.1-alpine`.
...
**Runtime stage.** A single `apk add` installs the fetch toolchain that the **low side** shells out to. The Go toolchain itself is already present from the `golang:1.27.1-alpine` base and is not re-installed via apk:
