...
- **Private git, APT, RPM, Alpine, and Conda upstreams** work the same way with `ARTIGATE_UPSTREAM_AUTH` (keyed by the mirror URL's exact host, `host:port` included) or the one-shot `auth` field on those collects; URLs embedding `user:pass@` are rejected.
- **Private Go module hosts** use their own standing variable, `ARTIGATE_GO_AUTH` (same `host=user:password` format), or the one-shot `auth` field, injected into the `go`/`git` subprocesses (per-collect netrc + git credential helper) and added to `GOPRIVATE` for that collect. Go does not read `ARTIGATE_UPSTREAM_AUTH` — a standing Go credential also marks its host private, which a git/APT login on a shared host must never do. SSH-key auth via a preconfigured `~/.ssh` still works too.
