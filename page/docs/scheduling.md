...
Every schedulable ecosystem page — from [Go](ecosystems/go.md) and [Python](ecosystems/python.md) through [Conda](ecosystems/conda.md), [Git](ecosystems/git.md), and [OSV](ecosystems/osv.md); everything in the [ecosystems overview](ecosystems/index.md) except [Uploads](ecosystems/uploads.md) — has a **"Schedule the above"** row beneath its collect form. Fill in the collect form as you would for a one-off export, then:
...
- On **create**, `spec` is sent as a raw JSON object (not a string) and stored verbatim; `label` defaults to `stream` when blank; the watch is always created enabled. The body limit is 8 MiB. Validation rejects unknown streams, intervals below 1 minute, empty or invalid-JSON specs, and specs carrying an `auth` key (credentials must never be stored — use `ARTIGATE_CONTAINER_AUTH` / `ARTIGATE_GO_AUTH` / `ARTIGATE_UPSTREAM_AUTH`) with HTTP 400. The same `auth` check runs again at **run time**, so a spec stored before this guard existed fails its runs with the same message instead of ever using the stored login — recreate the watch without credentials and put the login in the standing-credential variable.
