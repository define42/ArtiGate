...
- **External binaries.** `xz` must be installed on **both** sides for `.xz` metadata; `gpgv` on the low side for repomd verification; `gpg` on the high side for signing. Each download has a 10-minute timeout and a 4 GiB response cap.
