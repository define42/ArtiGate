...
- APT support mirrors binary `deb` packages for the configured suite/components/architectures; `deb-src`, `Contents-*`, `Translation-*`, and by-hash indexes are not mirrored. By default only the **newest** version of each package (per architecture) is mirrored — set `newest_only: false` (or untick "Newest version only" in the UI) to mirror every version in the index. The high side regenerates `Packages`/`Release` from the packages actually present; publish signed with `--apt-gpg-key` or have clients trust the repo explicitly.
- RPM support mirrors a repository's full metadata (`primary`, `filelists`, `other`, `updateinfo`, `comps`, `modules` — carried verbatim) plus its `.rpm`s; the high side regenerates and re-signs `repomd.xml`. By default only the **newest** EVR of each package (per architecture) is mirrored, and the `primary` index is rewritten to advertise only those packages (`newest_only: false` mirrors every version). Requirements/limits: `baseurl` must be concrete (`$releasever`/`$basearch` rejected); the `primary` index must be readable as plain/`.gz`/`.xz` to enumerate packages (a zchunk-*only* `primary` isn't parseable; newest-only additionally cannot rewrite a `.zck`-only `primary`, so disable newest-only for such repos); each collect is a full re-sync (no incremental delta yet). Publish signed with `--rpm-gpg-key` or set `repo_gpgcheck=0` on clients.
