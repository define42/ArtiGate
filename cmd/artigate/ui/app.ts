...
// openRepoGuide.
function condaGuideSection(base: string): GuideSection {
  return {
    heading: "Conda packages",
    body:
      "Each mirrored channel is served under its mirror name (the tree's " +
    ...
    blocks: [
      {
        ...
        code:
          `conda install --override-channels -c ${base}/conda/<mirror> <package>\n` +
    ...
    note:
      "repodata.json is rebuilt on this host from each package's verified " +
      "repodata entry; only packages that are actually present are listed.",
  };
}

function rubygemsGuideSection(base: string): GuideSection {
  return {
    heading: "Ruby gems",
    body:
      "The mirror serves the compact index Bundler and modern RubyGems " +
    ...
    note:
      "/versions and /info/<gem> are regenerated on this host from the " +
...
function composerGuideSection(base: string): GuideSection {
  return {
    heading: "PHP Composer packages",
    body:
      "The mirror is a Composer v2 repository: point repositories at it and " +
    ...
    note:
      "Package metadata is served from the signed bundle records and each " +
      "dist URL points back at this host's verified zip archives.",
  };
}

function vsxGuideSection(base: string): GuideSection {
  return {
    heading: "VS Code extensions",
    body:
      "The mirror answers the VS Code gallery API, so VSCodium and Code-OSS " +
      "builds can search and install extensions from it.",
    blocks: [
      {
        label: "Point the gallery at the mirror (VSCodium)",
        code:
          `export VSCODE_GALLERY_SERVICE_URL=${base}/vsx/gallery\n` +
    ...
    note:
      "Gallery metadata is regenerated from each extension's own embedded " +
      "package.json; only extensions whose .vsix is present are listed.",
  };
}

function galaxyGuideSection(base: string): GuideSection {
  return {
    ...
    note:
      "Version metadata (and each artifact's SHA-256 the client verifies) is " +
      "regenerated from the mirrored tarballs' own MANIFEST.json.",
  };
}

function cranGuideSection(base: string): GuideSection {
  return {
    heading: "R packages (CRAN)",
    body:
      "The mirror is a source CRAN repository: src/contrib with a PACKAGES " +
    ...
    note:
      "Only source packages are mirrored (type = \"source\"); R builds them " +
      "locally like it does against any CRAN mirror.",
  };
}

function snapGuideSection(base: string): GuideSection {
  return {
    heading: "Snap packages",
    body:
      "Each mirrored snap is served as the .snap/.assert pair `snap download` " +
    ...
    blocks: [
      {
        ...
        code:
          `curl -fLO ${base}/snap/files/<name>/<name>_<rev>.snap\n` +
    ...
    note:
      "The .assert carries the store's own signatures, verified by snapd " +
...
function gitGuideSection(base: string): GuideSection {
  return {
    heading: "Git repositories",
    body:
      "Each mirrored repository is served read-only over git's dumb HTTP " +
    ...
    note:
      "The pack index is rebuilt on this host from the verified packfile; " +
