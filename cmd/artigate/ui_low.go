...
<main>
  ...
  <div class="card">
    ...
    <p class="hint">List images to mirror &mdash; one reference per line, e.g. <code>alpine:3.20</code>, <code>ghcr.io/org/app:v1</code>, or <code>registry.access.redhat.com/ubi9/ubi@sha256:&hellip;</code>. The tag position also takes a <b>version constraint</b>, resolved to the newest matching numeric tag at collect time: <code>golang:1.26.x</code>, <code>golang:&lt;2.0.0</code>, or <code>golang:&gt;=1.24, &lt;2.0</code> (variant tags like <code>1.26.3-alpine</code> are never picked &mdash; pin those explicitly). Only <b>linux/amd64</b> is fetched. Attached <b>cosign signatures, in-toto/SLSA attestations, and SBOMs</b> mirror automatically (via the OCI referrers API, cosign's <code>sha256-&hellip;.sig</code> tags, and buildkit index entries), and the high side answers <code>/v2/&hellip;/referrers/&hellip;</code> so cosign, Kyverno, and Ratify verify against the mirror. Each upstream registry keeps its own namespace on the high side (<code>docker.io/&hellip;</code>, <code>ghcr.io/&hellip;</code>), so sources never mix. Same as POSTing to <code>/admin/containers/collect</code>.</p>
