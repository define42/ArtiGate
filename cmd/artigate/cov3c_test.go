...
// -----------------------------------------------------------------------------
// ui.go: cachedTree / listGoModules / goDetail error branches
// -----------------------------------------------------------------------------
...
func TestCov3C_CachedListsAndTreeError(t *testing.T) {
	...
	// Break the Go module tree so listGoModules errors, surfacing as a 500
	// from handleUITree and a direct cachedTree error.
	...
	if _, err := hs.cachedTree(t.Context(), goEcosystem()); err == nil {
		t.Error("cachedTree should error on an unreadable go module dir")
	}
