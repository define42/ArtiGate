...
func (s *HighServer) serveDiode(w http.ResponseWriter, r *http.Request) bool {
	...
	if base := strings.TrimSuffix(strings.TrimSuffix(strings.TrimSuffix(name, ".tar.gz"), ".manifest.json.sig"), ".manifest.json"); bundleCompleteInDir(s.cfg.Landing, base) {
		go func() {
...
// uploadBundleIfConfigured pushes a freshly exported (or re-exported) bundle
// to the HTTP diode endpoint, when one is configured. It is best-effort by
// design: the bundle is already committed and archived, so a failed upload
// loses nothing — it is reported (result, progress, log) and the staged files
// stay in the export dir for a re-transmit from the Status page.
func (s *LowServer) uploadBundleIfConfigured(ctx context.Context, res *ExportResult) {
	if s.cfg.DiodeURL == "" || res.BundleID == "" {
		return
	}
	...
	// The transfer is done: clear the outbound spool like a folder diode
	// moving the files out would. The archive copy is what re-exports use.
	for _, suffix := range bundleSuffixes() {
		if err := os.Remove(filepath.Join(s.cfg.ExportDir, res.BundleID+suffix)); err != nil && !os.IsNotExist(err) {
			log.Printf("diode upload %s: clear outbound: %v", res.BundleID, err)
		}
	}
}
